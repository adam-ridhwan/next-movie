...
export default async function Headshots({ id, contentType }: ContentRouteParams) {
  ...
  return (
    <ol className='hide-scrollbar flex flex-row overflow-x-auto'>
      <div className='min-w-leftRightCustom' />
      {firstTenActors.map((actor: TODO, i: number) => (
        <li key={actor.id} className={cn('flex flex-col gap-4', { 'pr-4': i !== firstTenActors.length - 1 })}>
          <div className='relative aspect-[4/5] h-48 overflow-hidden rounded-2xl bg-muted/50 shadow-tileShadow'>
            {actor.profile_path ? (
              <Image
